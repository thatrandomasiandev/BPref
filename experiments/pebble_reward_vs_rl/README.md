...
**Cursor → Colab kernel (preferred with the Colab extension)**  
1. Open `experiments/pebble_reward_vs_rl/colab_a100.ipynb` in Cursor  
2. **Select Kernel → Connect to Colab** → sign in → GPU (A100 if offered)  
3. Run cells in order. Cell 4 builds Python 3.11 + pinned `gym==0.26.2`

**Browser fallback**  
1. Fresh copy: [colab_a100.ipynb](https://colab.research.google.com/github/thatrandomasiandev/BPref/blob/main/experiments/pebble_reward_vs_rl/colab_a100.ipynb)  
2. Runtime → GPU → A100; delete runtime first if pip already failed  
