#!/usr/bin/env python3
"""
PEBBLE training loop instrumented for reward-quality vs RL-amplification.

Paper: Lee, Smith & Abbeel, ICML 2021 (arXiv:2106.05091) — see PAPER_NOTES.md
       and lee2021pebble.pdf in this folder.

Open question
-------------
When PEBBLE's *true* return drops under the paper's own ablations
(Fig. 5a: no-relabel / no-pretrain; Fig. 3: low feedback), is it because
reward-model ranking worsens (H1) or because SAC amplifies a still-decent
r̂ (H2)?

Teacher: scripted Oracle only (§5.1). No B-Pref Mistake / Stoc channels.

Conditions (set via `pebble_condition` + matching Hydra overrides):
  full / no_relabel / no_pretrain / low_budget

Learning rule is unchanged PEBBLE; diagnostics are additive.
"""
...
def _assert_oracle_teacher(cfg) -> None:
    """Refuse to silently run B-Pref noise under this experiment."""
    eps_m = float(cfg.teacher_eps_mistake)
    beta = float(cfg.teacher_beta)
    if eps_m > 0:
        raise ValueError(
            f"This experiment is PEBBLE-native (Oracle teacher only). "
            f"Got teacher_eps_mistake={eps_m}. Use B-Pref separately."
        )
    # PEBBLE / BPref code uses beta=-1 for deterministic Oracle ranking.
    if beta > 0:
        raise ValueError(
...
class PebbleDiagWorkspace:
    """PEBBLE Workspace with longitudinal Acc / return / causal-probe logs."""
    ...
    def __init__(self, cfg):
        ...
        # Paper ablation tag (full / no_relabel / no_pretrain / low_budget).
        condition = str(getattr(cfg, "pebble_condition", "full"))
        # Fig. 5a: skip buffer rewrite after reward updates.
    ...
    def _maybe_relabel(self) -> None:
        """PEBBLE §4.3 relabel — gated for the no_relabel ablation."""
        if self.do_relabel:
            self.replay_buffer.relabel_with_predictor(self.reward_model)
        else:
            print("[pebble] skipping buffer relabel (no_relabel ablation)")
    ...
    # ------------------------------------------------------------------
    # Reward learning (paper Alg. 2) + diagnostic hook
    # ------------------------------------------------------------------
    ...
    def run(self):
        ...
        while self.step < self.cfg.num_train_steps:
            ...
            # ============================================================
            # End of pretrain (or end of seed if no_pretrain) → first prefs
            # ============================================================
